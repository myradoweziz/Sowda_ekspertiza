<template>
	<div class="inner-news">
		<div class="inner-news__container __container">
            ...
            <div class="inner-news__infos-num" v-if="selectedNews?.views_count">{{ `${selectedNews.views_count || ''}` }}</div>
          </div> -->
					<div class="inner-news__infos-viewed">
						<div class="inner-news__infos-icon">
...
<script>
	...
	import { getNews, getMainNewsId } from '@/api/user.api'
	import { mapGetters } from 'vuex'
	export default {
		components: { NewsItem },
		computed: { ...mapGetters(['imgURL']) },
		data() {
			return { limit: 4, page: 0, news: [], selectedNews: {} }
		},
		async mounted() {
		...
		methods: {
			async fetchNews() {
				try {
					const { data, status } = await getMainNewsId({ data: { id: this.$route.params.id } })
