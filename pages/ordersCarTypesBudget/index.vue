    ...
    <div class="car-types__container __container">
      ...
      <div class="news-page__body">
        <div class="car-types__items">
          <div v-for="item in budgetCars" :key="item.UUID" @click="openContact(item)" class="car-types__item">
            ...
            <div class="car-types__bottom">
              ...
              <div class="car-types__image">
                <img src="@/assets/img/car-order.png" alt="car">
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  import { getOrderBudget } from '@/api/user.api'
  ...
  export default {
    ...
    async mounted() {
      await this.fetchBudgetCars()
    },
    methods: {
      openContact(uuid) {
        this.$cookie.set('budgetItemsUuid', uuid)
        this.$router.push('/ordersBudgetContact')
      },
      async fetchBudgetCars() {
        try {
          const { data, status } = await getOrderBudget({ data: { v: this.bus, e: this.yuridiki, b: this.budget } })
          if (status) {
            this.budgetCars = data.data
          }
        } catch (error) {
          console.error(error)
        }
