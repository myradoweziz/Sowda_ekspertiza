		...
		<div class="table__container __container">
			<div class="table__body">
				...
				<div class="table__row">
					<div class="table__title">
						<p>{{ $t('numberSeats') }}:</p>
						<span>{{ form?.contacts.countBus }}</span>
					</div>
				</div>
				<div class="table__row">
					<div class="table__title">
...
<script>
	...
	import { sendBusPerson } from '@/api/user.api'
	...
	export default {
		...
		computed: {
			calculated() {
				if (this.form?.contacts?.countBus >= this.items?.quantity) {
					let sum = Math.round(this.form?.contacts.countBus / this.items?.quantity)
					let galyndy = Math.round(this.form?.contacts.countBus % this.items?.quantity)
					if (galyndy != 0) {
						sum = sum + 1
					}
					return sum * this.items?.price * Number(this.form?.contacts.delay)
				} else {
					return this.items?.price * Number(this.form?.contacts?.delay)
				}
			},
			amount() {
		...
		data() {
			return { form: null, items: null, busPersons: null, loading: false }
		},
		async mounted() {
			const getForm = await this.$cookie.get('formTable')
		...
		methods: {
			async fetchSendBus() {
				try {
					this.loading = true
					const { data, status } = await sendBusPerson({
						data: {
							...
							quantity: Number(this.form.contacts.countBus),
