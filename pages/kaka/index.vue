<template>
  <div class="company-b">
    <div class="company-b__container __container">
        ...
        <div class="company-b__service">
          <div class="company-b__service-box">
            <div class="company-b__service-itemBox">
              <div
                v-for="item in passengers"
                :key="item.name"
                @click="openPassenger(item.id)"
                class="company-b__service-item"
              >
                <div class="company-b__service-appendIcon">
                  <base-icon :name="item.icon" />
                </div>
                <div class="company-b__service-text">{{ $t(item.name) }}</div>
                <div class="company-b__service-prependIcon">
                  <base-icon :name="item.lastIcon" />
                </div>
              </div>
            </div>
          </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        page: 0,
        passengers: [
          { id: 0, icon: 'truckMini', name: 'title', lastIcon: 'chevronRight' },
          { id: 1, icon: 'busMini', name: 'label', lastIcon: 'chevronRight' }
        ],
        form: {
          name: '',
          email: '',
    ...
    methods: {
      openPassenger(id) {
        if (id === 0) {
          this.$router.push(this.localeLocation('/busPassengers'))
        }
        if (id === 1) {
          this.$router.push(this.localeLocation('/passenger'))
        }
      },
      async sendMessage() {
        try {
      ...
      async fetchCity() {
        try {
          ...
          await this.fetchCommutes(this.cities[2].uuid)
          await this.fetchCompanies(this.cities[2].uuid)
          await this.fetchContact(this.cities[2].uuid)
        } catch (error) {
          console.error(error)
        }
      },
