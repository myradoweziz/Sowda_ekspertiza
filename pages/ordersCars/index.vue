					...
					<div class="orders-cars__service-itemBox">
						<div class="orders-cars__service-content">
							<div :class="['orders-cars__service-item', { active: activeCheck }]" @click="openCheckedFirst">
								...
								<div class="orders-cars__service-text">{{ $t('bus') }}</div>
							</div>
							<div :class="['orders-cars__service-item', { active: isActive }]" @click="openCheckedSecond">
								...
								<div class="orders-cars__service-text">{{ $t('truck') }}</div>
							</div>
						</div>
						<div class="orders-cars__bottom">
							<div class="orders-cars__button">
								<Button @onClick="sendDocuments" :text="$t('next')" padding appendIcon="west" primary />
							</div>
						</div>
					</div>
				</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			sendDocuments() {
				if (this.activeCheck) {
					this.$router.push('/personalOrders')
				}
				if (this.isActive) {
					this.$router.push('/personalTruckOrders')
				}
			}
		}
	}
