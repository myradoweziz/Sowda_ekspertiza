...
<script>
  ...
  export default {
    components: { BaseIcon },
    data() {
      return { orderCars: [], bus: 1, fiziki: 1 }
    },
    async mounted() {
    ...
    methods: {
      openContact(item) {
        this.$cookie.set('items', item)
        this.$router.push('/ordersContact')
      },
      async fetchCars() {
        try {
          const { data, status } = await getOrderList({ data: { v: this.bus, e: this.fiziki } })
