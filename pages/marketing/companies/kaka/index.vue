	...
	<div class="transport">
		<div class="transport__container">
			<div class="transport__title-box">
				<div class="transport__title">Kaka</div>
			...
			<div class="transport__body">
				<div class="transport__table">
					<table>
						...
						<tbody>
							<tr v-for="(item, index) in enterprices" :key="item.uuid">
								<td>
									<img :src="`${imgURL}/uploads/enterprises/${item.img_path}`" alt="" />
								</td>
...
<script>
	...
	import { getEnterprices, getCities, addEnterprices, deleteEnterprices } from '@/api/admin.api'
	...
	export default {
		...
		methods: {
			...
			changeCities(idx) {
				if (idx == 1) {
					this.$router.push('/marketing/companies/kaka/services')
				}
				if (idx == 2) {
					this.$router.push('/marketing/companies/kaka/communicate')
				}
			},
			selectEnterprice(item) {
				this.form = item
				this.openPopup()
			},
			async getCity() {
				try {
					...
					await this.getEnterprice(this.cities[2].uuid)
				} catch (error) {
					console.error(error)
				}
			},
			async getEnterprice(id) {
				try {
					const { data, status } = await getEnterprices({ data: { uuid: id } })
					if (status) this.enterprices = data.data
				} catch (error) {
					console.error(error)
				}
			},
			async saveEnterprice(form) {
				try {
					const { data, status } = await addEnterprices({
						data: {
							file: form.img_path,
			...
			async deleteSelected(id) {
				try {
					const { status } = await deleteEnterprices({
						data: { uuid: id }
					})
