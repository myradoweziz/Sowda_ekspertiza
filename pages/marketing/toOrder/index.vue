...
<script>
	...
	import { getToBusOrders } from '@/api/admin.api'
	...
	export default {
		...
		methods: {
			...
			async fetchToOrders() {
				try {
					const { data, status } = await getToBusOrders({ data: { l: this.limit, p: this.page } })
					if (status) this.orders = data.data.orders_bus
				} catch (error) {
					console.error(error)
				}
			}
