<template>
	<div class="transport">
		<div class="transport__container">
			...
			<popup-travelers travel v-if="activePopup" @close="closePopup" @add="addTravel" @save="saveTravel" :form="form" />
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeCities(idx) {
				if (idx == 0) {
					this.$router.push('/marketing/travelers/')
				}
				if (idx == 2) {
			...
			async getCity() {
				try {
					...
					await this.getTravel(this.cities[1].uuid)
				} catch (error) {
					console.error(error)
				}
			},
